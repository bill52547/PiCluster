...
def parallel_with_error_detect(tasks, rate_of_tolerance=3):
    ...
    def _task_with_avg_time_with_tolerance(task_tuple: "(List[Output], List[datetime.timedelta])"):
    ...
    output_with_time = (rx.combine_latest(start_time, tasks_with_time_stamp)
                        .pipe(ops.map(_get_time_used),
                              ops.scan(_tuple_acc, ([], []))))

    _outputs_with_time_of_tolerance_on_majority_completed = output_with_time.pipe(
        ops.filter(lambda t: len(t[1]) > num_tasks * 0.8),
        ops.map(_task_with_avg_time_with_tolerance)).pipe(ops.share())

    _time_of_tolerance_on_majority_completed = _outputs_with_time_of_tolerance_on_majority_completed.pipe(
        ops.map(lambda t: t[1]),
        ops.take(1),
        ops.flat_map(lambda time_of_tolerance: rx.Observable().pipe(ops.timeout(time_of_tolerance)))
    )
